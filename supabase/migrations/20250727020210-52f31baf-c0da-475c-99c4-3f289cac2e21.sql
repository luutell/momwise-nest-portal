...
BEGIN
  ...
  ELSE
    ...
    target_week_offset := (target_date - user_baby_birth_date) / 7;
  END IF;
  
  target_day_of_week := EXTRACT(DOW FROM target_date); -- 0 = domingo
  ...
  WHERE 
    ...
    AND (cc.day_of_week IS NULL OR cc.day_of_week = target_day_of_week)
  ORDER BY 
    CASE WHEN cc.week_offset = target_week_offset THEN 1 ELSE 2 END,
    cc.created_at DESC
  LIMIT 1;
END;
$function$;